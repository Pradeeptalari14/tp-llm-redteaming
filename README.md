...
[![Studio](https://img.shields.io/badge/Developer_Studio-Live-brightgreen)](https://pradeeptalari14.github.io/portfolio/tools/llm-redteaming/)
...
| Tab | Description |
|-----|-------------|
| `garak-config.yaml` | Generated in studio Output tab |
| `run-audit.sh` | Generated in studio Output tab |
| `Flow Diagram` | Generated in studio Output tab |

### Configurable Options
| Option | Available Values |
|--------|-----------------|
| **Probe Type** | `Jailbreak (DAN)` / `Injection (XSS)` / `Data Leak (PII)` / `Factuality (Hallucination)` |
| **Test Suite Size** | `50` / `100` / `250` / `500` |
| **Threshold Tolerance** | `80%` / `90%` / `95%` / `99%` |
...
```mermaid
graph TD
  Probe[🎯 Security Probe] -->|Generate adversarial tests| Runner[⚡ Garak Vulnerability Runner]
  Runner -->|Send prompt attempts| Target[🤖 Target LLM Model]
  Target -->|Return responses| Eval[🔍 Alignment Evaluator]
  Eval -->|Checks fail| CI[❌ Exit 1: Block CI Pipeline]
  Eval -->|Checks pass| HTML[✅ Save HTML Report]
```
...
```bash
git clone https://github.com/Pradeeptalari14/tp-llm-redteaming.git
cd tp-llm-redteaming
```
...
```bash
kubectl port-forward svc/tp-llm-redteaming 8501:8501
```
...
# Primary Config File: garak-config.yaml
```
...
### Probe Type
- **`Jailbreak (DAN)`** — see `examples/` folder for generated output
- **`Injection (XSS)`** — see `examples/` folder for generated output
- **`Data Leak (PII)`** — see `examples/` folder for generated output
- **`Factuality (Hallucination)`** — see `examples/` folder for generated output

### Test Suite Size
- **`50`** — see `examples/` folder for generated output
- **`100`** — see `examples/` folder for generated output
- **`250`** — see `examples/` folder for generated output
- **`500`** — see `examples/` folder for generated output

### Threshold Tolerance
- **`80%`** — see `examples/` folder for generated output
- **`90%`** — see `examples/` folder for generated output
- **`95%`** — see `examples/` folder for generated output
- **`99%`** — see `examples/` folder for generated output
...
| Resource | Link |
|----------|------|
| Interactive Studio | [Open →](https://pradeeptalari14.github.io/portfolio/tools/llm-redteaming/) |
...
---
*Generated by [LLM Red Teaming & Vulnerability Scanner Studio Studio](https://pradeeptalari14.github.io/portfolio/tools/llm-redteaming/) — [Talari Pradeep Portfolio](https://pradeeptalari14.github.io/portfolio)*